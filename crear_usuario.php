      ...
      <!--------------------------
		...
		<div class="container">
		  <ul class="nav nav-tabs">
			<li class="active"><a data-toggle="tab" href="#home">Crear Usuario</a></li>
			<li><a data-toggle="tab" href="#menu1">Borrar Usuario</a></li>
		  </ul>
		  ...
		  <div class="tab-content">
			<div id="home" class="tab-pane fade in active">
				...
				<h4>Usuario</h4>
				<input id='usuario' class="form-control form-control-lg" type="text" placeholder="Usuario" style="width: 220px;">
				<h4>Contrase&ntilde;a</h4>
				<input type="password" id="password" name="password" class="form-control" data-toggle="password">
				<button id='guardar_usuario' type="button" class="btn btn-danger" style="width: 220px; margin-top: 20px;">Guardar</button>
			</div>
...
<script type="text/javascript" src="dist/js/bootstrap-show-password.min.js"></script>
<script>
	...
	*/
	...
	$("#contenedorUsuarios" ).delegate( ".borrar", "click", function() {
		
		$("#contenedorUsuarios").html("<p style='text-align:center; font-size:20px; margin-top:50px;'>cargando...</p>");
		
		var id_usuario = $(this).attr('id');
		
		$.ajax({
			url: '/borrar_usuario.php?id_usuario=' + id_usuario,
			type: 'GET',
	...
	function cargar_usuarios(){
		...
		$.ajax({
		url: '/obtener_usuario_y_empresa.php',
		type: 'GET',
		success: function(resultado){ 
			var usuarios = Object.values(JSON.parse(resultado));
			var cuerpo_tabla = ``;
			$.each( usuarios, function( i, item ) {
				cuerpo_tabla += `    <tr>
										...
										<td>${item.empresa}</td>
										<td>
											<button type="button" id="${item.id_usuario}" class="btn btn-danger borrar">Borrar</button>
										</td>
									</tr>
								`;
			});
			...
			$("#contenedorUsuarios").html(table_html);
	...
	$("#guardar_usuario").on("click",function(){
			...
			if(empresa === null || usuario === "" || password === ""){
				alert("Todos los campos son obligatorios!");
			}else{
				var peticion ='/guardar_usuario.php?id_empresa=' + empresa + '&usuario=' + usuario + '&contrasena=' + password;
				
				$.ajax({
				url: peticion,
				type: 'GET',
				success: function(resultado){ 
					if(resultado === "Usuario insertado con exito"){
						$("#usuario").val('');
						$("#password").val('');
						cargar_usuarios();
					}
