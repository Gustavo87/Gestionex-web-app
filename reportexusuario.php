      ...
      <ul class="sidebar-menu" data-widget="tree">
		...
		<li class="active treeview">
		  ...
		  <ul class="treeview-menu">
            ...
            <li class="active"><a href="reportexusuario.php">Por Usuario</a></li>
          </ul>
        </li>
	   <!--		
	   <li class="header"></li>
  ...
  <div class="content-wrapper">
    ...
    <section class="content container-fluid">
      ...
      <!--------------------------
		...
		<h4>Usuario</h4>		
		<div id='contenedor_filtro_usuario'></div>		
		<h4>Fecha</h4>
...
<script>
		...
		$('#filtrar').on('click',function(){
			...
			idIntervaloActualizacion = setInterval(function(){
        			...
        			var url = "/obtener_reportexusuario.php?id_usuario="+ usuario +"&fecha_inicial="+ fechain +"&fecha_final=" + fechafi;	
			...
			$("#datosReporte tbody").html("<tr><td colspan='6' style='text-align:center'>cargando...</td><tr>");
		
			    usuario = $('#select_usuario').val();
			...
			var url_pdf = "http://www.app.gestionex.co//obtener_reportexusuario_pdf.php?id_usuario="+usuario+"&fecha_inicial="+fechain+"&fecha_final="+fechafi;
			var url_excel = "http://www.app.gestionex.co//obtener_reportexusuario_excel.php?id_usuario="+usuario+"&fecha_inicial="+fechain+"&fecha_final="+fechafi;
			...
			success: function(resultado){ 
				var filas = Object.values(JSON.parse(resultado));
				if(filas.length === 0 ){
					$("#datosReporte tbody").html("<tr id='filamensaje' ><td colspan='6' style='text-align:center'>No hay datos para esta selecci&oacute;n...</td><tr>");
				}else{
			...
			error: function(data) {
				...
				$( "#contenedor_filtro_empresa" ).delegate( "#select_empresa", "change", function() {	
					$("#contenedor_filtro_usuario").html('cargando filtro usuarios...');
					$.ajax({
						url: '/obtener_usuariosxempresa.php?id_empresa=' + $('#select_empresa').val(),
						type: 'GET',
						success: function(resultado){ 
							var empresas = Object.values(JSON.parse(resultado));
							var select_html = "<select id='select_usuario' class='selectpicker' data-live-search='true'>";
							$.each( empresas, function( i, item ) {
								select_html+="<option value='" + item.id + "' data-tokens='ketchup mustard'>" + item.usuario + "</option>";
							});
							select_html+="</select>";
							$("#contenedor_filtro_usuario").html(select_html);
							$("#select_usuario").selectpicker('refresh');
							
						},
						error: function(data) {
							alert('Error de red, no se cargo el filtro de usuarios, favor intente otra vez.');
						}
					});					
				});
